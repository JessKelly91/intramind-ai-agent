...
The output keeps full `retrieved_contexts` for deterministic retrieval metrics and records `synthesis_contexts` for the smaller context set actually sent to the answer synthesizer. It also records eval parameters (`collection`, `num_results`, `min_score`, fixture PII redaction mode, vectorizer mode, prompt label, and prompt versions) so baseline changes can be traced to either retrieval settings or prompt changes.
